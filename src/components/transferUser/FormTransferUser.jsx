...
import {
  validateDPI,
  validateDPIMessage,
} from "../../shared/validator/validateDPI";
import {
  validateAccount,
  validateAccountMessage,
} from "../../shared/validator/validateAccount";
import {
  validateDescriptionTransfer,
  validateDescriptionTransferMessage,
  validateMaxTransfer,
  validateMaxTransferMessage,
...
export const FormTransferUser = () => {
  ...
  const handleTransfer = (event) => {
    event.preventDefault();
    postTransfer(
      (form.noOwnerAccount.value = selectAccount),
      form.noDestinationAccount.value,
      parseInt(form.DPI_DestinationAccount.value),
  ...
  const buttonDisabled =
    !form.DPI_DestinationAccount.isValid ||
    !form.noDestinationAccount.isValid ||
    !form.amount.isValid;
            ...
            {Array.isArray(accounts) &&
              accounts.map((account, index) => (
                <option
                  key={index}
                  value={
                    account !== "Select account" ? account.slice(11) : account
                  }
                  className="input-text-transfer-user"
                >
